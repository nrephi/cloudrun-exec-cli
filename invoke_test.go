...
func TestScriptHanlderErrorsNoBucket(t *testing.T) {
	original := os.Getenv("CLOUDRUN_EXEC_CLI_ARCHIVE_BUCKET")
	os.Unsetenv("CLOUDRUN_EXEC_CLI_ARCHIVE_BUCKET")
	defer os.Setenv("CLOUDRUN_EXEC_CLI_ARCHIVE_BUCKET", original)

	req := httptest.NewRequest("GET", "/", nil)
	rr := httptest.NewRecorder()
	logBytes := callHandler(scriptHandler, rr, req, t)

	want := "gcloud-report: 'CLOUDRUN_EXEC_CLI_ARCHIVE_BUCKET' not found"
...
func TestScriptHandler(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input: "",
			want:  fmt.Sprintf("Wrote report to gs://%s/report-.-", os.Getenv("CLOUDRUN_EXEC_CLI_ARCHIVE_BUCKET")),
		},
		{
			// Test a valid name string can be used.
			input: "name",
			want:  fmt.Sprintf("Wrote report to gs://%s/report-name-", os.Getenv("CLOUDRUN_EXEC_CLI_ARCHIVE_BUCKET")),
		},
		{
